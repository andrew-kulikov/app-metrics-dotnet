...
namespace AppMetrics.Examples.Framework.Console
{
    public static class Host
    {
        private const string InfluxDbDatabase = "metricsinfluxdbsandboxconsole";
        private const string InfluxDbUri = "http://localhost:8086";
        ...
        {
            foreach (var unused in Enumerable.Range(0, 10)) RecordMetrics();
        ...
        private static async Task RunUntilEscAsync(TimeSpan delayBetweenRun,
            CancellationTokenSource cancellationTokenSource, Func<Task> action)
        {
